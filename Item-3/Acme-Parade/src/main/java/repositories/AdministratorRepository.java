...
public interface AdministratorRepository extends JpaRepository<Administrator, Integer> {
	...
	Collection<Parade> query5(Date date);
	...
	@Query("select count(f)*1.0 / (select count(f1)*1.0 from Finder f1 where f1.parades.size > 0) from Finder f where f.parades.size = 0")
	Double query11();
	...
	@Query("select c from Chapter c join c.area a join a.brotherhoods b where b.parades.size >= (select  avg(b1.parades.size)*0.1 + avg(b1.parades.size) from Chapter c1 join c1.area a1 join a1.brotherhoods b1)")
	Collection<Chapter> query17();

	@Query("select count(p)*1.0 / (select count(p1)*1.0 from Parade p1 where p1.draftMode = false) from Parade p where p.draftMode = true")
	Double query18();

	@Query("select count(p)*1.0 / (select count(p1)*1.0 from Parade p1), p.status from Parade p where p.draftMode = false group by p.status")
